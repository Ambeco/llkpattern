...
- **More scraped-corpus sources planned** beyond OpenJDK, RE2J and AOSP — dregex, dk.brics.automaton, and DataDog/java-reggie are identified candidates. The RE2J corpus turned up a match-time crash (`((x))*`) and an ambiguity-check gap (`a*^a`); see remaining_work.md.
...
The module compiles; its test suite passes fully: **3149 tests, 0 failing** — hand-written unit/integration/differential tests, plus **2646 tests from a scraped-corpus differential harness** (compares `Ll1Pattern` against real test data mined from OpenJDK's own `java.util.regex` test suite (561 rows) RE2J's tests (1790 rows) and AOSP libcore's extra rows (295, mostly the original ASCII `TestCases.txt`); more corpus sources are planned, see above) and 2646 further reference-only checks (re-verifying `java.util.regex`'s own recorded behavior against the installed JDK) that are disabled by default, hence "skipped" rather than run. See [documents/remaining_work.md](documents/remaining_work.md) for the JDK version required to run the suite and the full TODO list, and [documents/notes.md](documents/notes.md) for the bugs this harness has already found and fixed.
