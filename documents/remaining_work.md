...
- [ ] Remaining `Matcher` API gaps: `useAnchoringBounds`/`hasAnchoringBounds`, `useTransparentBounds`/`hasTransparentBounds` (see the `region()` item below) -- all still `UnsupportedOperationException` stubs. Also `find(int)` doesn't `reset()` first the way `java.util.regex.Matcher#find(int)` does, and a failed `find()` doesn't stay failed (the next `find()` restarts from `regionStart`).
