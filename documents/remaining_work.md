...
- [ ] **Extend `needsEntryPointBeforeMatcher()` to the quantified-loop case** (`QuantifiedUnion`/`ComplexQuantifiedCharacter` when actually a loop, not just their bare-flags/unquantified cases) -- their `buildMatcher()` (via `buildLoopMatcher` → `DispatchMatcherConstruct`'s loop-flavored constructor) independently recomputes the body/`next` merge rather than reading anything `buildEntryMap()` cached, so the same skip should be safe there too. Not yet done or measured -- see design.md's "`needsEntryPointBeforeMatcher()`" section for why the leaf/`Sequence` case alone was enough to move the measured number.
