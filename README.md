...
- **Compilation** (`PatternConstruct` → `MatcherConstruct`): working for the non-repeating subset of the grammar — literals, character classes, sequences, and plain (unquantified) alternation, including real ambiguity detection (two `|` branches that could match the same next character are a compile-time error). Quantifiers that require an actual loop (`?`, `*`, `+`, `{n,m}` on a group or character class) are not yet compiled — that's the next piece of work, and it's also where the design's cycle-handling mechanism (see [documents/design.md](documents/design.md)) actually gets exercised.
