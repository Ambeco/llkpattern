...
- [ ] Implement `MatcherConstruct.BoundaryMatcherConstruct.match(...)` for `BoundaryConstruct`'s `BoundaryEnum` values (`LineBegin`/`LineEnd`/`InputBegin`/`PreviousMatchEnd`/`InputEndExceptTerminator`/`InputEnd`/`Linebreak` — currently throws). `\b`/`\B` are a separate, already-implemented `WordBoundaryConstruct`/`WordBoundaryMatcherConstruct` pair — see design.md's "Boundary matching" section for that design, which these likely want to reuse (e.g. `^` under `MULTILINE` needs `Matcher#peekPrevious()` to check the preceding character is a line terminator).
