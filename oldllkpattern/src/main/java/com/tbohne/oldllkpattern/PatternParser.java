...
class PatternParser {
  ...
  QuantifiedUnion parse() {
    QuantifiedUnion root = parseUnion(new QuantifiedUnion(0, flags));
      ...
      if ("()[]|.^$\0".indexOf(peek) > -1) {
        ...
        switch (peek) {
          ...
          case '|':
            if (sequence.patterns.isEmpty()) {
              ...
              parent.patterns.add(sequence.patterns.get(0));
              sequence = new Sequence(index);
              advance(1);
            } else {
              sequence.endIndex = index;
              parent.patterns.add(sequence);
              sequence = new Sequence(index);
              advance(1);
            }
            break;
          case '.':
            sequence.patterns.add(parseQuantifiable(RegexCharacterClass.DOT.get(flags)));
            advance(1);
            break;
          case '^':
        ...
        if (codePoint != -1) {
          ...
          if (boundaryConstruct != null) {
            ...
            sequence.patterns.add(parseQuantifiable(parseComplexEscape(new SimpleCharacterClass(index))));
          }
        }
      } else {
        int startIndex = index;
        ...
        if (peek == '{' || peek == '?' || peek == '+' || peek == '*') {
          ...
          SimpleCharacterClass complex = new SimpleCharacterClass(startIndex).add(fullChar);
  ...
  private QuantifiedUnion parseGroup() {
    ...
    QuantifiedUnion union = new QuantifiedUnion(index, flags);
    advance(1);
    if (peek == '?') {
      advance(1);
      switch (peek) {
    ...
    if (index == pattern.length()) {
      throw throwUnexpectedChar(
          "expected \")\" to match ", (CodePointReference) () -> union.startIndex);
    }
    if (peek != ')') {
  ...
  private SimpleCharacterClass parseComplexCharacter() {
    if (peek != '[') {
      throw new IllegalStateException("entered parseComplexCharacter at illegal start point");
    }
    SimpleCharacterClass complex = new SimpleCharacterClass(index);
    advance(1);
    if (peek == '^') {
      complex.positive = false;
      advance(1);
    }
    for (; ; ) {
      switch (peek) {
        case '\0':
          throw throwUnexpectedChar("expected \"]\" to match ", (CodePointReference) () -> complex.startIndex);
        case ']':
          if (index > complex.startIndex + 1) {
            complex.endIndex = index;
            return complex;
          } else {
            complex.directMatches.append(']');
            advance(1);
            break;
          }
        case '-':
        ...
        case '\\':
          ...
          if (eCodePoint != -1) {
            complex.directMatches.appendCodePoint(eCodePoint);
          } else {
        ...
        case '&':
          if (index + 5 < pattern.length()
            ...
            advance(2);
            complex.intersections.add(parseComplexCharacter());
            break;
          }
          // fallthrough
        default:
          ...
          if (peek == '-') {
            ...
            complex.directMatches.appendCodePoint(codePoint);
          }
      }
    }
  }
  ...
  private SimpleCharacterClass parseComplexEscape(SimpleCharacterClass complex) {
    if (peek != '\\') {
      throw new IllegalStateException("entered parseComplexEscape at illegal start point");
    }
    advance(1);
    if (peek == 'R') {
      complex.add(RegexCharacterClass.R.get(flags));
      advance(1);
      complex.endIndex = index;
      return complex;
    }
    if (peek != 'p' && peek != 'P') {
      try {
        complex.add(RegexCharacterClass.valueOf(Character.toString(peek)).get(flags));
        advance(1);
        complex.endIndex = index;
        return complex;
    ...
    for (; ; ) {
      if (++end == pattern.length()) {
        throw throwUnexpectedChar(
            "character class ", (CodePointReference) () -> index, " is missing the closing }");
      }
    ...
    // Blocks:   InX, block=X,  blk=X
    // java:   javaX
    // POSIX:   X
    // Categories:   IsX,  general_category=X,  gc=X,  X
    // U+Properties:   IsX
    // Scripts:   IsX, script=X,  sc=X
    if (prefix == NamedCharClass.CharacterClassPrefix.in || prefix == NamedCharClass.CharacterClassPrefix.block) {
      // definitely a block. Try this before the expensive named character classes
      try {
        complex.endIndex = index;
        return complex.add(Character.UnicodeBlock.forName(charClassName));
      } catch (IllegalArgumentException ignored) {
        throw throwUnexpectedChar("unknown Unicode block \"", charClassName, "\"");
      }
    }
    // try all the named character classes. This can be a little expensive, but is also the common
    // case.
    if (prefix != NamedCharClass.CharacterClassPrefix.script) {
      try {
        complex.endIndex = index;
        return complex.add(NamedCharClass.valueOf(charClassName).get(flags));
      } catch (IllegalArgumentException ignored) {
        if (prefix == NamedCharClass.CharacterClassPrefix.java) {
          throw throwUnexpectedChar(
              "unknown java.lang.Character character class \"", charClassName, "\"");
        } else if (prefix == NamedCharClass.CharacterClassPrefix.general_category) {
          throw throwUnexpectedChar("unknown Unicode category \"", charClassName, "\"");
        } else if (prefix == NamedCharClass.CharacterClassPrefix.none) {
          throw throwUnexpectedChar("unknown character class or property \"", charClassName, "\"");
        } else if (prefix == NamedCharClass.CharacterClassPrefix.is) {
          // for "Is" prefix, it might still be a script, so we fall through
        }
      }
    }
    if (prefix != NamedCharClass.CharacterClassPrefix.script && prefix != NamedCharClass.CharacterClassPrefix.is) {
      throw new IllegalStateException(
          "This should never happen. Compiler should always have thrown an error by "
              + "this point except for is-prefixed character classes");
    }
    try { // It might be a script, but otherwise must be an unknown character class with no prefix
      complex.endIndex = index;
      return complex.add(Character.UnicodeScript.forName(charClassName));
    } catch (IllegalArgumentException ignored) {
      if (prefix == NamedCharClass.CharacterClassPrefix.script) {
        throw throwUnexpectedChar("unknown Unicode script \"", charClassName, "\"");
      } else if (prefix == NamedCharClass.CharacterClassPrefix.is) {
        throw throwUnexpectedChar(
            "unknown POSIX character class, Unicode category, or Unicode Property \"",
            charClassName,
            "\"");
      }
    }
    throw new IllegalStateException(
        "This should never happen. All flows should have returned or thrown");
  }

  private void parseMaybeRangePredicate(SimpleCharacterClass complex, int startCodePoint) {
    if (peek != '-') {
      throw new IllegalStateException("entered parseComplexCharacter at illegal start point");
    }
    advance(1);
    if (peek == ']') {
      complex.directMatches.appendCodePoint(startCodePoint);
      complex.directMatches.append('-');
    } else if (peek == '\\') {
      ...
      complex.ranges.add(new CharRange(startCodePoint, endCodePoint));
    } else {
  ...
  private ComplexQuantifiedCharacter parseQuantifiable(SimpleCharacterClass construct) {
      ...
      if (peek == ',') {
        ...
        if (peek != '}') {
          throw throwUnexpectedChar(
              ...
              (CodePointReference) () -> startQuantifierIndex);
        }
        advance(1);
      }
      construct.endIndex = index;
  ...
  private PatternSyntaxException throwUnexpectedChar(Object... expectations) {
    Object[] args =
        concatObjectArrays(new Object[] {"Unexpected ", (CodePoint) () -> peek}, expectations);
  ...
  private PatternSyntaxException throwEmptySequence(int sequenceStartIndex, int unionStartIndex) {
    if (sequenceStartIndex != unionStartIndex && unionStartIndex > 0) {
      throw throwGenericPatternSyntaxException(
          ...
          (CodePointReference) () -> sequenceStartIndex,
          " inside group started at ",
          (CodePointReference) () -> unionStartIndex);
