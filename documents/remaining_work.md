...
- [ ] Eliminate the remaining Guava `RangeSet<Integer>` usage now that `CodePointMap.complement` supports what it needs: `PatternConstruct.ComplexCharacter.ranges`/`validRanges()`, `PatternConstruct.firstCharSet`/`lastCharSet`/`WordBoundaryConstruct.priorCharSet`, `MatcherConstruct.containsFolded`/`SingleCharMatcherConstruct.validRanges`/`WordBoundaryConstruct`'s `wordSet`, `PatternParser`'s `RangeSet`-building parse logic (`.complement()` for DOT/negation, `&&` intersection via `a.removeAll(b.complement())`), and `NamedCharClass`/`UnicodePredicates`'s `ImmutableRangeSet` constants. Plan: migrate `ComplexCharacter` and the match-time hot path (`containsFolded`) onto `CodePointMap<Boolean>` directly; leave the `unicodeanalyzer`-generated `UnicodePredicates.java` and its generator untouched, converting each `ImmutableRangeSet` constant to `CodePointMap<Boolean>` lazily (not eagerly/statically, to avoid `NamedCharClass`'s documented static-init circularity — see notes.md) at the `NamedCharClass` boundary instead. Also delete `CharacterClass.java` (confirmed dead: zero subclasses/instantiations anywhere in main or test source) as part of this pass.
