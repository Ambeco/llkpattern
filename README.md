...
**Corpus compile time (each pass compiles 406 patterns):**
...
**flatten-matcher-dispatch experiment branch (2026-09-18):** the Intel-i7-9750H row above is NOT
re-measured this session — a concurrent session and an open browser made desktop timing too noisy
to trust (allocation sampling, which isn't timing-sensitive, showed the change as a wash — see
notes.md). The Pixel 3a row above IS from this experiment, and shows a real (if modest) compile
and match speedup over the pre-experiment baseline (compile: 5.76 -> 5.55ms/pass; match: 0.77 ->
0.76ms/pass) — though `compileRegex`'s own unchanged-code number also moved between runs (6.21 ->
7.68ms/pass), confirming real run-to-run device noise even here, so treat this as a promising
early read, not a settled result. A quiet-desktop re-run of the Intel row is still needed before
deciding whether this experiment merges to main.
