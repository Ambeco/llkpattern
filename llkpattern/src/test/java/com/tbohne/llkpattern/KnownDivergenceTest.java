...
public class KnownDivergenceTest {
  ...
  // --- Lookahead is rejected by design: it can't be guaranteed to run in linear time. Lookbehind
  // is too, EXCEPT when its body always matches exactly one code point (see LookbehindTest) -- a
  // direct generalization of \b/\B's own single-code-point peekPrevious() check.

  @Test
  public void lookaheadIsRejectedByDesign() {
    for (String p : new String[] {"(?=a)a", "(?!b)a"}) {
      assertRejected(p, 0);
    }
  }

  @Test
  public void multiCodePointLookbehindIsRejectedByDesign() {
    for (String p : new String[] {"(?<=ab)c", "(?<!ab)c"}) {
      assertRejected(p, 0);
    }
  }
  ...
  // --- A reluctant modifier followed only by a CONDITIONAL zero-width construct (one that can
  // succeed mid-run, not just at true end-of-input/region-end -- \b, \B, MULTILINE ^/$) now stops
  // as soon as it's genuinely satisfiable, matching java.util.regex -- MatcherConstruct's
  // exitAssertionChain (generalizing the old exitIsPureEnd) evaluates the assertion chain itself,
  // via peek/peekPrevious, before committing to the early exit. See design.md's "Quantifier/loop
  // compilation" section and ReluctantQuantifierDifferentialTest, which exercises these same
  // shapes across a full input/region matrix, not just the one case each asserted below.

  @Test
  public void reluctantLoopBeforeWordBoundaryStopsAssoonAsSatisfiable() {
    // \B is already satisfiable right after the first 'a', since both neighbors -- 'a' and 'a' --
    // are word characters.
  ...
  @Test
  public void reluctantLoopBeforeMultilineDollarStopsAssoonAsSatisfiable() {
    // The MULTILINE $ is first satisfiable right before the '\n'.
