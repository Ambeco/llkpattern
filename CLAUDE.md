...
1. Re-run the relevant JMH benchmark(s) on this desktop (`./gradlew :llkpattern:jmh`, JDK 17/21 for
   ...
   `documents/benchmarks/corpus_benchmark_results.json` and README.md's benchmark tables with the
   new numbers.
2. If the change plausibly shifts *where* time is spent (not just how much), re-capture CPU
   sampling too (temporary `profilers = ['gc', 'stack:lines=8;detailLine=true']` in
   `llkpattern/build.gradle`, reverted after) and update the relevant
   `documents/benchmarks/Intel-i7-9750H_*_sampling.txt` file(s) -- stale sampling naming a
...
3. Prompt the user to plug in and unlock the Pixel 3a so the on-device numbers
   (`documents/benchmarks/Google_Pixel_3a_sargo_corpus_benchmark_results.json`) can be re-run and
